...
void render(const std::vector<Sphere>& spheres)
{
    ...
    for (size_t i = 0; i < w_width; ++i)
    {
        for (size_t j = 0; j < w_height; ++j)
        {
            ...
            pixelInfo[i + j * w_width] = cast_ray(Vec3f(0.f,0.f,0.f), dir, spheres);
        }
    }
...
Vec3f cast_ray(const Vec3f& orig, const Vec3f& dir, const std::vector<Sphere>& spheres) {
    Vec3f material;
    if (!pixel_depth_check(orig, dir, spheres, material)) {
...
bool pixel_depth_check(const Vec3f& orig, const Vec3f& dir, const std::vector<Sphere>& spheres, Vec3f& material)
{
